from black_scholes import black_scholes_greeks, black_scholes_call, black_scholes_put, calcular_volatilidade
import datetime
import pandas as pd
...
MY_TICKERS = ['VALE3', 'PETR4', 'WEGE3', 'RADL3', 'BBAS3', 'VIVT3',
              ...
              'TAEE11', 'CSAN3', 'ODPV3', 'FESA4', 'BBSE3',
              'AZUL4', 'BOVA11']
...
def gera_base_opcoes(lista_acoes=None):
    ...
    with st.spinner('Black & Scholes'):
        ...
        # black_scholes_put
        df['Black & Scholes Put'] = df.apply(lambda row: black_scholes_put(row['cotacao'], row['strike'], row['dias'], r, row['volatilidade_implicita']), axis=1)

        # black_scholes_call
        df['Black & Scholes Call'] = df.apply(lambda row: black_scholes_call(row['cotacao'], row['strike'], row['dias'], r, row['volatilidade_implicita']), axis=1)

        df.loc[(df.tipo == 'PUT'), 'bs'] = df['Black & Scholes Put']
        df.loc[(df.tipo == 'CALL'), 'bs'] = df['Black & Scholes Call']
        df['bs'] = round(df['bs'], 2)

        df.drop(columns=['Black & Scholes Put', 'Black & Scholes Call'], inplace=True)

    with st.spinner('Greeks'):
...
def main():
    ...
    df_aux.columns = [
        ...
        'Preg', 'Delta', 'Volat', 'B&S', 'Bid', 'Ask', 'Neg', 'Total',
